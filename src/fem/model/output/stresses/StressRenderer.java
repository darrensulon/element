...
public class StressRenderer implements Renderer {

	private AbstractStressRecorder m_recorder;
	...
	public StressRenderer(AbstractStressRecorder hsr){
		this.m_recorder = hsr;
		max = hsr.maxAbsHeatState();
	}
	
	private Shape arrow(StressPoint hsp, double zps){
	...
	private Shape tensionArrow(StressPoint hsp, double zps){
		Vector hs = hsp.getStress();
		Vector crds = hsp.getCrds();
		double _l = hs.abs();
		double _a = Math.atan2(hs.get(1),hs.get(0));
		if(((StandardStressRecorder)m_recorder).getMode() == ElasticGraphicalOutputOptions.S_INPLANE2) {
			_a = hs.get(2);
			_l = hs.get(1);
		}else if(((StandardStressRecorder)m_recorder).getMode() == ElasticGraphicalOutputOptions.S_INPLANE1) {
			_a = hs.get(2);
			_l = hs.get(0);
		}
		double _scale = max_length/max*zps;
	...
	private Shape compressionArrow(StressPoint hsp, double zps){
		Vector hs = hsp.getStress();
		Vector crds = hsp.getCrds();
		double _l = hs.abs();
		double _a = Math.atan2(hs.get(1),hs.get(0));
...
//		g2d.setColor(_color);
		...
		while(iter.hasNext()){
			StressPoint ntv = iter.next();			
			Vector crds = ntv.getCrds();
			Point2D _p2d = new Point2D.Double(crds.get(0),crds.get(1));
			zp.ucs2pixel(_p2d,_p2d);
			
			if(((StandardStressRecorder)m_recorder).getMode() == ElasticGraphicalOutputOptions.S_12) {
			...
			}else if(((StandardStressRecorder)m_recorder).getMode() == ElasticGraphicalOutputOptions.S_11||
					((StandardStressRecorder)m_recorder).getMode() == ElasticGraphicalOutputOptions.S_22) {
				Shape arrow = arrow(ntv,zp.getScale());
				g2d.setStroke(new BasicStroke(2));
				g2d.setColor(Color.CYAN);
				g2d.draw(zp.transformShapeToPanel(arrow));		
			}else if (((StandardStressRecorder)m_recorder).getMode() == ElasticGraphicalOutputOptions.S_INPLANE1) {
				if(ntv.getStress().get(0)<=0) {
					Shape arrow = compressionArrow(ntv,zp.getScale());
					g2d.setStroke(new BasicStroke(2));
					g2d.setColor(Color.RED);
					g2d.draw(zp.transformShapeToPanel(arrow));
				}else {
					Shape arrow = tensionArrow(ntv,zp.getScale());
					g2d.setStroke(new BasicStroke(2));
					g2d.setColor(Color.CYAN);
					g2d.draw(zp.transformShapeToPanel(arrow));
				}
				
			}else if (((StandardStressRecorder)m_recorder).getMode() == ElasticGraphicalOutputOptions.S_INPLANE2) {
				if(ntv.getStress().get(1)<=0) {
					Shape arrow = compressionArrow(ntv,zp.getScale());
					g2d.setStroke(new BasicStroke(2));
					g2d.setColor(Color.RED);
					g2d.draw(zp.transformShapeToPanel(arrow));
			...
			if(((StandardStressRecorder)m_recorder).getMode() == ElasticGraphicalOutputOptions.S_INPLANE1) {
				layout = new TextLayout(new AttributedString(df.format(ntv.getStress().get(0))).getIterator(),g2d.getFontRenderContext());
			}else if(((StandardStressRecorder)m_recorder).getMode() == ElasticGraphicalOutputOptions.S_INPLANE2) {
				layout = new TextLayout(new AttributedString(df.format(ntv.getStress().get(1))).getIterator(),g2d.getFontRenderContext());
			}
